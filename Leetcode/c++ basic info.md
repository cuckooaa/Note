...
```
1. 变量名和 `auto`：`auto` 关键字用于类型推导。编译器会根据 Lambda 表达式的定义自动推导出其实际类型（它是一个匿名的函数对象类）。
2. 捕获列表 (Capture List)：这是 Lambda 表达式最关键的部分，它决定了 Lambda 内部如何访问它**外部作用域**中的变量。
...
3. 参数列表 (Parameters)：这指定了函数被调用时接受的参数类型和名称。这与普通函数的参数列表完全相同。
4. mutable(optional) 默认const属性：对于一个非mutable的lambda，编译器会将其视为对const对象的成员函数调用。
5. 函数体和返回类型：`{...}`：这是 Lambda 函数实际执行的代码块。
